...
- **일정 편집**: 달력에서 날짜를 클릭하여 연차/오프 추가, 수정, 삭제 가능.
- **데이터 검증 및 표준화**:
...
이 애플리케이션은 **PWA (Progressive Web App)** 기술이 적용되어 있어, 앱스토어 다운로드 없이 브라우저에서 바로 앱으로 설치할 수 있습니다.
**Chrome 브라우저**에 최적화되어 있으므로 Chrome 사용을 권장합니다.

#### 🖥️ 데스크탑 (Chrome)
1. Chrome 브라우저로 접속합니다.
2. 주소창 우측의 **설치 아이콘** (📥 또는 모니터 모양)을 클릭합니다.
3. **'설치'** 버튼을 누르면 바탕화면에 아이콘이 생성되고 독립된 앱처럼 실행됩니다.

#### 🤖 안드로이드 (Chrome)
1. Chrome 브라우저로 접속합니다.
2. 우측 상단 **더보기 (⋮)** 메뉴를 터치합니다.
3. **'앱 설치'** 또는 **'홈 화면에 추가'** 메뉴를 선택합니다.
4. 설치가 완료되면 앱 서랍과 홈 화면에 아이콘이 생성됩니다.

#### 🍎 아이폰 (Chrome)
1. Chrome 브라우저로 접속합니다.
2. 주소창 우측 상단의 **공유 아이콘** (사각형 위 화살표)을 터치합니다.
3. 메뉴 목록에서 **'홈 화면에 추가'**를 찾아 선택합니다.
4. 이름을 확인하고 **'추가'**를 누르면 홈 화면에 설치됩니다.
